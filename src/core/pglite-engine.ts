...
export class PGLiteEngine implements BrainEngine {
  ...
  private async applyForwardReferenceBootstrap(): Promise<void> {
    ...
    // v0.42.56.0 (v121): the schema blob indexes event_page_id before the
    // numbered migration can add it on an existing timeline_entries table.
  ...
  async getHealth(): Promise<BrainHealth> {
    ...
    const { rows: [h] } = await this.db.query(`
      ...
      SELECT
        ...
        -- Graph scoring is meaningful only for active entity-like pages.
