/**
 * runCycle exit hook writes last_source_cycle_at after successful per-source
 * maintenance. It writes legacy last_full_cycle_at only when ALL_PHASES were
 * selected, so a partial cycle cannot claim full-cycle completion.
 *
...
// source freshness exit hook silently no-ops. Each test wraps its
...
describe('runCycle source freshness exit hook', () => {
  test('partial phase selection stamps source freshness without claiming a full cycle', async () => {
    await withEnv({ GBRAIN_HOME: gbrainHome }, async () => {
      await seedSource('alpha');
      expect(await readLastSourceCycleAt('alpha')).toBeNull();
  ...
  test('legacy caller (no sourceId) does NOT write any source timestamp', async () => {
    await withEnv({ GBRAIN_HOME: gbrainHome }, async () => {
      ...
      expect(await readLastSourceCycleAt('default-like')).toBeNull();
      expect(await readLastFullCycleAt('default-like')).toBeNull();
    });
  });
  ...
  test('dryRun=true skips the write', async () => {
    await withEnv({ GBRAIN_HOME: gbrainHome }, async () => {
      ...
      expect(await readLastSourceCycleAt('beta')).toBeNull();
  ...
  test('cycle that returns skipped (lock held) does NOT mark timestamp', async () => {
    await withEnv({ GBRAIN_HOME: gbrainHome }, async () => {
      ...
      expect(await readLastSourceCycleAt('gamma')).toBeNull();
      expect(await readLastFullCycleAt('gamma')).toBeNull();
    });
  });
