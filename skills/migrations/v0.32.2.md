...
3. Before any filesystem write, preflights every source that actually has
   pending fence writes. A missing required `local_path` fails the whole
   batch without recreating that source root; uncommitted changes in a
   write target also fail the batch. Registered sources with no pending
   rows do not block this migration. This mirrors the v0.14.1 `dry-fix`
   safety posture while keeping unrelated work out of the gate.
